...
import loginAfterHover from "../../assets/images/loginAfterHover.svg";
import login  from "../../assets/images/login.svg";
import qlogo from "../../assets/images/qlogo.svg";
import error from "../../assets/images/error.svg";
import pwdOpenIcon from "../../assets/images/pwdOpenIcon.svg";
// import pwdCloseIcon from "../../assets/images/pwdCloseIcon.svg";
// import pwdOpenDark from "../../assets/images/pwdOpenDark.svg";
...
const Forgotpassword = () => {
  ...
  const sendMail = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://worksoftbuddyapi.qualesce.com/api/v1/users/forgotPassword",
        {
          method: "POST",
      ...
      if (response.ok) {
        ...
        setTimeout(() => navigate("/"), 2500); // Smooth transition after success
      } else {
        const result = await response.json();
  ...
  return (
    <>
      <div className={`forgot_pwd_container flex flex-col min-h-screen items-center justify-between w-screen  ${isDark ? "bg-[#121212] text-white" : "bg-white text-black"}`}>
        <div className="flex-grow flex items-center justify-center w-full">
          <div className="w-[400px]">
            <form onSubmit={sendMail} className="px-8 rounded-2xl" autoComplete="off">
              ...
              {/* Email Input */}
              <div>
                <label htmlFor="email" className="block py-0.5 text-sm sm:text-base md:text-base lg:text-base">
                  Email:
                </label>
                <input
                  type="text"
                  ...
                  className={`w-full pr-0.5 py-1 outline-none border-b-1 ${isDark ? 'border-[white]' : 'border-[#283E46]'}`}
                />
              ...
              <div className="mt-4 flex items-center flex-col gap-0.5 text-xs sm:text-base md:text-base lg:text-base">
                <p>
                  ...
                  <Link
                    to="/"
                    className={`underline text-[#283E46] font-semibold text-xs sm:text-base md:text-base lg:text-base ${isDark ? 'text-[#ffc300]' : 'text-[#283E46]'}`}
                  >
                    Sign in{" "}
                  </Link>
                </p>
